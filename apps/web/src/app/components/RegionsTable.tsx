...
export function RegionsTable() {
  ...
  const getFlag = (loc: string) => {
    if (loc.includes('India') || loc.includes('Mumbai')) return 'https://lh3.googleusercontent.com/aida-public/AB6AXuA2IU2AbHqdfuGBAWGuh6Tj4QPR398D2d7N0QgHb3DVYF3pnO9fxvxEvY54RoRHoOdn46VdIn7mdTsY7QuS2-ehG8jjJAlnguHhWjZgPv6vl44iKCjt8SUC-QFqFsv_JYuMgJN0hTht_0DmXekw1hJInTdHdSBvf_mNeoKpwnitDjTCupNAQWkEMr8z9FfDKqWWQQEe1QRfw_qJh5TQ4_RLAetKmXChBp2YrWzd-PXJHIFTV_GnD-v3Jvqlsj2fcS0yimu5mpNuthzj';
    if (loc.includes('US') || loc.includes('Virginia')) return 'https://lh3.googleusercontent.com/aida-public/AB6AXuCMc8xximJjpF30mznsoZr7oJTa6-eZvOf40HSbNTxcEGcvnPRjV-ZT0YiHsgc4jZ3La5n3NqAJ8iBZAkc3vqzAwOvKzVxfFKrGyuc2M2IYEvM0XQAQp35WPm-7Hqy5Su3m2xF0hq_DvUqMruYCeD4mHRPJbqTsbj7GAuVKGdzW4tzvgyM8488N3TXU_2WS-ZZ71f1wBh3YGh3HbFuJerh04T1NxB-Bgm3j8MD4rUaXO_mOqH61Z01ToZeCBEE6ojEBwyrrKz9qh0ej';
    if (loc.includes('Sweden') || loc.includes('Stockholm')) return 'https://lh3.googleusercontent.com/aida-public/AB6AXuCMP4LPdH3yNVrimNbx_aaBo6RSE1n7ysIOuR0l9F4RRQo1G0fy2FcH-_c1qcH_x8_ON-iCJaffz6h4WW8frmy9-Ti1-fQIsy3MGej6AWFNR6cfLte5xxrR_bLPS2VBb61rS1AXHHgiwMtS3SBPMd7tRhP6_ukAZLG13YhJVytmb_CBIlN3HCabuq-HFnUpdSHjDBp3lqxl44Y7faGx4NlDJAjhr2CbOCKRTZxY5jJOYpaLw5riBXL7WZW1wd3VHZU4r5b7CnxHNYUv';
    return ''; // Fallback
  };
    ...
    <div className="overflow-x-auto">
      <table className="w-full border-collapse">
        ...
        <tbody className="divide-y divide-outline-variant">
          {regions.map((region) => {
            const rating = getRating(region.carbonIntensity);
            const isBest = region.carbonIntensity < 50;
            const mix = region.energyMix || {};
            ...
            return (
              <tr key={region.id} className={`${isBest ? 'bg-primary-container/5 border-l-4 border-primary-container' : 'hover:bg-surface-container transition-colors group'}`}>
                <td className={`py-lg ${isBest ? 'pl-lg' : ''}`}>
                  <div className="flex items-center gap-md">
                    <div className="w-8 h-5 rounded-sm bg-surface-container-highest overflow-hidden">
                      {getFlag(region.location) && (
                        <img alt={`Flag of ${region.location}`} className="w-full h-full object-cover" src={getFlag(region.location)} />
                      )}
                    </div>
                ...
                <td className={`py-lg font-code ${isBest ? 'text-primary-container' : (rating.label === 'CRITICAL' ? 'text-error' : 'text-on-surface-variant')}`}>
                  {region.carbonIntensity} gCO2/kWh
                </td>
                <td className="py-lg">
                  <div className="flex h-3 w-48 bg-surface-container-highest rounded-full overflow-hidden">
                    {mix.coal && <div style={{ width: `${mix.coal}%` }} className="h-full bg-error" title="Coal" />}
                    {mix.gas && <div style={{ width: `${mix.gas}%` }} className="h-full bg-error-container" title="Gas" />}
                    {mix.nuclear && <div style={{ width: `${mix.nuclear}%` }} className="h-full bg-primary-container/60" title="Nuclear" />}
                    {mix.renewables && <div style={{ width: `${mix.renewables}%` }} className="h-full bg-primary-container" title="Renewables" />}
                    {mix.other && <div style={{ width: `${mix.other}%` }} className="h-full bg-tertiary-container" title="Other" />}
                  </div>
                </td>
