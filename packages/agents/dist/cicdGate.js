...
exports.runGateAgent = runGateAgent;
const core_1 = require("@carbonix/core");
const core_2 = require("@carbonix/core");
/**
 * Parse Terraform diff for aws_instance / google_compute_instance blocks
 */
function parseTerraformResources(diff) {
    const resources = [];
 ...
 */
function parseDockerResources(diff) {
    const resources = [];
    const addedLines = diff.split('\n').filter(line => line.startsWith('+') && !line.startsWith('+++'));
    for (const line of addedLines) {
 ...
 */
async function runGateAgent(diff, budgetKgPerDay = 10) {
    ...
    for (const resource of allResources) {
        try {
            ...
            const result = await (0, core_1.calculateCarbon)({
            ...
            const gridIntensity = core_2.DEFAULT_GRID_INTENSITIES[resource.region] || 400;
