...
async function withRetry(fn, label) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            ...
            if (err instanceof agents_1.PlatformAuthError)
                throw err; // Auth errors don't retry
            lastErr = err;
            const backoffMs = 500 * Math.pow(2, attempt - 1); // 500ms, 1s, 2s
            console.warn(`[CRON] ${label}: transient error (attempt ${attempt}/3), retrying in ${backoffMs}ms:`, err.message);
...
node_cron_1.default.schedule('0 * * * *', async () => {
    ...
    for (const project of projects) {
        ...
        try {
            ...
            if (project.dataSource === 'LIVE' && project.platformTokens.length > 0) {
                ...
                for (const pt of project.platformTokens) {
                    try {
                        let plainToken;
                        try {
                            plainToken = (0, platformTokenService_1.decryptToken)(pt.encryptedToken);
                        }
                        catch (decErr) {
                            console.error(`[CRON] Cannot decrypt token for project "${project.name}" / ${pt.platform}: ${decErr.message}`);
                            continue;
                        }
                        const records = await withRetry(() => (0, agents_1.collectFromPlatform)({ platform: pt.platform, decryptedToken: plainToken, projectSlug: pt.projectSlug || undefined }), `${project.name}/${pt.platform}`);
                        // Update verification timestamp on success
                        await prisma_1.prisma.platformToken.update({
                            where: { id: pt.id },
                        ...
                        if (err instanceof agents_1.PlatformAuthError) {
                            ...
                            console.error(`[CRON] Auth failure for "${project.name}" / ${pt.platform} — marking INVALID:`, err.message);
                            await prisma_1.prisma.platformToken.update({
                                where: { id: pt.id },
                                data: {
                                    status: 'INVALID',
                            ...
                            const activeCount = await prisma_1.prisma.platformToken.count({
                                where: { projectId: project.id, status: 'ACTIVE' },
                            });
                            if (activeCount === 0) {
                                await prisma_1.prisma.project.update({ where: { id: project.id }, data: { dataSource: 'NO_CREDS' } });
                            }
                            ...
                            console.error(`[CRON] All retries failed for "${project.name}" / ${pt.platform}:`, err.message);
                            await prisma_1.prisma.platformToken.update({
                                where: { id: pt.id },
                                data: { lastError: err.message, failCount: { increment: 1 } },
                            });
                        }
                    }
                }
                ...
                const mockResult = await (0, agents_1.runCollector)(true);
            ...
            console.error(`[CRON] Unexpected error processing project "${project.name}":`, projectErr.message);
        }
    }
    ...
    try {
        ...
        const analystResult = await (0, agents_1.runAnalyst)(allRecords, NVIDIA_API_KEY);
        await prisma_1.prisma.agentRun.update({
        ...
        console.error('[CRON] Analyst pipeline failed:', error.message);
    }
});
...
node_cron_1.default.schedule('0 0 1 * *', async () => {
    ...
    try {
        ...
        const result = await (0, agents_1.runReporter)(records.map(r => ({
            instanceId: r.instanceId,
            instanceType: r.instanceType,
